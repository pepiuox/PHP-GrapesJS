...
class OldCRUD {
    ...
    public function __construct() {
        global$conn, $uconn, $econn, $dbdata, $rname;
		
		if(isset($_SESSION['databs'])){
			if($_SESSION['databs'] === 1 ){
				$this->conn = $conn;
				$this->baseDB = 'app_cms';
			}elseif($_SESSION['databs'] === 2){
				$this->conn = $conn;
				$this->baseDB = 'app_users';
			}elseif($_SESSION['databs'] === 3){
				$this->conn = $econn;
				$this->baseDB = 'app_ecommerce';
			}
		}else{
			echo 'Not exist Session;';
		}
        $this->dbdata = $dbdata;
        $this->pgname = $rname;
