...
class config {
	...
	private function files_check($local_config, $storage_path, $storage_config, $user_config, $user_valid) {
		...
		if ((file_exists(config::$config['root']) && !is_writable(config::$config['root'])) || (file_exists(config::$config['storage_path']) && !is_writable(config::$config['storage_path'])))
			exists_writeable(__FILE__, _basename(__FILE__));
		// extension_loaded
		if (function_exists('extension_loaded'))
			foreach (['gd', 'exif', 'mbstring'] as $name)
		...
		if (function_exists('exec'))
			echo prop('ffmpeg', !!get_ffmpeg_path());
		// ini_get
	...
	private function x3_check() {
		if (empty(self::$config['root']) || !is_string(self::$config['root']))
			return;
		$path_arr = explode('/content', self::$config['root']);
		if (count($path_arr) < 2 || !@file_exists($path_arr[0] . '/app/x3.inc.php'))
			return;
		self::$x3_path = real_path($path_arr[0]);
		if (!self::$has_login)
			get_include('plugins/files.x3-login.php'); // optional x3 login plugin
	}
	...
	function __construct($is_doc = false) {
		...
		if ($image_cache || self::$config['cache']) {
			...
			if ($is_doc && self::$storage_config_realpath && (!file_exists(self::$storage_config_realpath) || filemtime(self::$storage_config_realpath) < filemtime(__FILE__)))
				self::save_config();
		...
		if ($is_doc)
			self::$assets = empty(self::$config['assets']) ? 'https://cdn.jsdelivr.net/npm/' : rtrim(self::$config['assets'], '/') . '/';
...
function check_login($is_doc) {
	...
	if (session_status() === PHP_SESSION_NONE && !session_start() && $is_doc)
...
function read_file($path, $mime = false, $msg = false, $props = false, $cache_headers = false, $clone = false) {
	...
	if ($msg)
		header('files-msg: ' . $msg . ($cloned ? ' [cloned to ' . _basename($clone) . ']' : '') . ' [' . ($props ? $props . ', ' : '') . header_memory_time() . ']');
	if ($cache_headers)
		set_cache_headers();
...
function is_exclude($path = false, $is_dir = true, $symlinked = false) {
	...
	if (strpos('/' . root_relative($path), '/_files') !== false)
		return true;
...
function valid_root_path($path, $is_dir = false) {
	...
	if (!$is_dir && empty($path))
		return; // path cannot be empty if file
	if ($path && substr($path, -1) == '/')
		return; // path should never be root absolute or end with /
...
function get_file($path, $resize = false) {
	...
	if ($resize == 'video') {
		...
		if ($mime && strtok($mime, '/') !== 'video')
			error('<strong>' . _basename($path) . '</strong> (' . $mime . ') is not a video.', 415);
		...
		if (!$ffmpeg_path)
			error('<a href="http://ffmpeg.org/" target="_blank">FFmpeg</a> disabled. Check your <a href="' . _basename(__FILE__) . '?check=1" target="_blank">diagnostics</a>.', 400);
		...
		if (file_exists($cache) && !filesize($cache) && imagejpeg(imagecreate(1, 1), $cache))
		...
		if ($mime && strtok($mime, '/') !== 'image')
			error('<strong>' . _basename($path) . '</strong> (' . $mime . ') is not an image.', 415);
...
function resize_image($path, $resize_dimensions, $clone = false) {
	...
	if ((!$is_resize_type || ($resize_ratio < max(config::$config['image_resize_min_ratio'], 1) && $file_size <= config::$config['load_images_max_filesize'])) && !read_file($path, $info['mime'], 'Original image served', $header_props, true, $clone))
...
function get_dir($path, $files = false, $json_url = false) {
	...
	if (!$realpath)
		return; // no real path for any reason
	...
	if ($symlinked && is_exclude($realpath, true, $symlinked))
		return; // exclude check again symlink realpath
...
function get_exif($path) {
	...
	foreach (array('DateTime', 'DateTimeOriginal', 'ExposureTime', 'FNumber', 'FocalLength', 'Make', 'Model', 'Orientation', 'ISOSpeedRatings', 'Software') as $name) {
		...
		if ($val)
			$exif[$name] = strpos($name, 'DateTime') === 0 ? exif_timestamp($val) : (is_string($val) ? trim($val) : $val);
	}
	...
	foreach (array('GPSLatitude', 'GPSLongitude') as $key) {
		...
		for ($i = 0; $i < 3; $i++) {
			...
			if (count($part) == 1) {
				...
				if ($part[1] == 0)
					return false; // can't be 0 / invalid GPS
...
function get_files_data($dir, $url_path = false, &$dirsize = 0, &$files_count = 0, &$images_count = 0, &$preview = false) {
	...
	foreach ($filenames as $filename) {
		...
		if (!$realpath)
			continue; // no real path for any reason, for example symlink dead
		$symlinked = $realpath !== $path; // path is symlinked at some point
		// filetype
		...
		if ($symlinked && is_exclude($realpath, $is_dir, $symlinked))
			continue; // exclude check again symlink realpath
...
// vars
		...
		if (!$item_url_path && $url_path)
			$item_url_path = $url_path . ($url_path === '/' ? '' : '/') . ($is_link ? _basename($path) : $basename);
		...
		if ($is_image) {
			...
			if (!empty($imagesize) && is_array($imagesize)) {
				...
				if ($item_url_path && $imagesize[0] && $imagesize[0] > 2048 && $imagesize[0] / $imagesize[1] === 2) {
					...
					foreach ([2048, 4096, 8192] as $resize) {
						if ($resize >= $imagesize[0])
							break;
						if (file_exists($dir . '/_files_' . $resize . '_' . $filename))
							$panorama_resized[] = $resize;
					}
					if (!empty($panorama_resized))
						$item['panorama_resized'] = array_reverse($panorama_resized);
				}
			...
			if (!empty($url_lines) && is_array($url_lines))
				foreach ($url_lines as $str)
					if (preg_match('/^url\s*=\s*([\S\s]+)/i', trim($str), $url_matches) && !empty($url_matches) && isset($url_matches[1])) {
...
function get_valid_menu_cache($cache) {
	...
	foreach ($arr as $key => $val) {
		...
		if (strpos($path, '/') !== false && $val['mtime'] !== @filemtime(root_absolute($path)))
			return; // skip shallow 1st level dirs, and compare filemtime
	}
	return $json;
}
...
if (post('action')) {
	...
	if ($action === 'fm') {
		...
		if (empty($path))
			json_error('invalid path ' . $post_path);
		$path = real_path($path); // in case of symlink path
		...
		function fm_json_exit($success, $arr) {
			...
			if ($x3 && @file_exists($x3) && @is_writable($x3))
				touch($x3);
			json_exit($arr);
		}
		...
		if ($task === 'upload') {
			...
			if (!$is_dir)
				json_error('invalid dir ' . $post_path);
			// upload path must be writeable
			if (!is_writable($path))
				json_error('upload dir ' . $post_path . ' is not writeable');
			// get $_FILES['file']
			...
			if (!$is_dir)
				json_error('invalid dir ' . $post_path); // parent path must be dir
			if (!is_writable($path))
				json_error($post_path . ' is not writeable.'); // dir must be writeable
			...
			if (!is_writable($path))
				json_error($post_path . ' is not writeable.'); // path must be writeable
			$name = name_is_allowed(post('name')); // trim and check valid
			$new_path = dirname($path) . '/' . $name;
			if (file_exists($new_path))
				json_error("$name already exists."); // new name exists
...
// security: prevent renaming 'file.html' to 'file.php' / file must already be *.php when renaming
			...
			if (!is_writable($parent_dir))
				json_error(_basename($parent_dir) . ' is not writeable.'); // dir must be writeable
			$name = name_is_allowed(post('name')); // trim and check valid
			...
			if ($success)
				@touch(dirname($path)); // invalidate any cache by updating parent dir mtime
		...
		if (!$version || version_compare($version, config::$version) <= 0 || !is_writable(__FILE__))
			json_error(); // requirements
		$get = @file_get_contents('https://cdn.jsdelivr.net/npm/files.photo.gallery@' . $version . '/index.php');
		if (empty($get) || strpos($get, '<?php') !== 0)
			json_error(); // basic validation
...
// GET
	...
	if (get('download_dir_zip')) {
		...
		if (!config::$storage_path || config::$config['download_dir_cache'] === 'dir') {
			if (!is_writable($dir))
				error('Dir ' . _basename($dir) . ' is not writeable.', 500);
		...
		if (!$cached) {
			...
			foreach ($files as $file)
				$zip->addFile($file, _basename($file));
...
// look for custom language files _files/lang/*.json
		function lang_custom() {
			...
			foreach ($files as $path) {
				...
				if (!empty($data))
					$langs[strtok(_basename($path), '.')] = $data;
			}
...
// htmlstart
		...
		foreach (array_filter([
			...
			echo '<script src="' . config::$assets . $key . '"></script>';
		?>
			</body>
		</html>
	<?php
