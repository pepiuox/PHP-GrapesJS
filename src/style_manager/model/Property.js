...
export default class Property extends Model {
  ...
  __upTargets(p, opts = {}) {
    ...
    const applyStyle =
      !opts.__up &&
      !parentProp &&
      (isClear || kProps.some((k) => toProps.indexOf(k) >= 0));
  ...
  canClear() {
    ...
    return parent
      ? parent.__canClearProp(this)
      : this.hasValue({ noParent: true });
  }
  ...
  __getFullValue({ withDefault } = {}) {
    return !this.hasValue() && withDefault
  ...
  __parseFn(input = '') {
    ...
    return {
      ...
      value: String.prototype.substring
        .apply(input, [start, end >= 0 ? end : undefined])
        .trim(),
    };
  }
