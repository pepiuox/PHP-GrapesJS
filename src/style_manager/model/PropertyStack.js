...
export default class PropertyStack extends PropertyComposite {
  ...
  __upSelected({ noEvent } = {}, opts = {}) {
    ...
    values &&
      this.getProperties().forEach((prop) => {
        ...
        prop.__getFullValue() !== value &&
          prop.upValue(value, { ...opts, __up: true });
      });
  ...
  __parseLayer(value) {
    ...
    return parseFn
      ...
      : properties.reduce((acc, prop, i) => {
          ...
          acc[prop.getId()] = !isUndefined(value)
            ? value
  ...
  __getLayersFromStyle(style = {}) {
    ...
    let result = fromStyle
      ? fromStyle(style, { property: this, name, separatorLayers: sep })
      : [];
      ...
      const layers = this.__splitStyleName(style, name, sep)
        ...
        .map((parts) => {
          ...
          props.forEach((prop, i) => {
            const value = parts[i];
            result[prop.getId()] = !isUndefined(value)
              ? value
              : prop.getDefaultValue();
          });
          return result;
