...
export default class PropertyComposite extends Property {
  ...
  getProperty(id) {
    return (
      this.get('properties').filter(
        (prop) => prop.getId() === id || prop.getName() === id
      )[0] || null
    );
  }
  ...
  __styleHasProps(style = {}) {
    ...
    return allNameProps.some(
      (prop) => !isUndefined(style[prop]) && style[prop] !== ''
    );
  }
  ...
  __getSplitValue(value = '', { byName } = {}) {
    const props = this.getProperties();
    const props4Nums =
      props.length === 4 && props.every((prop) => isNumberType(prop.getType()));
    ...
    props.forEach((prop, i) => {
      ...
      if (props4Nums) {
        ...
        res =
          values[i] || values[(i % len) + (len != 1 && len % 2 ? 1 : 0)] || res;
      }
  ...
  __getPropsFromStyle(style = {}, opts = {}) {
    ...
    let result = fromStyle
      ? fromStyle(style, { property: this, name, separator: sep })
      : {};
