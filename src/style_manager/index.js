...
export default () => {
  ...
  return {
    ...
    init(config = {}) {
      ...
      // Other listeners
      model.on('change:lastTarget', () =>
        em.trigger(evTarget, this.getSelected())
      );
    ...
    getProperty(sectorId, id) {
      ...
      if (sector) {
        prop = sector
          .get('properties')
          .filter(
            (prop) => prop.get('property') === id || prop.get('id') === id
          )[0];
      }
    ...
    getParentRules(target, { state, component } = {}) {
      ...
      if (em && target) {
        ...
        if (cmp) {
          ...
          otherRules = sel
            ? cssC.getRules(sel.getSelectors().getFullName(optsSel))
            : [];
    ...
    __upProp(prop, style, parentStyles, opts) {
      ...
      if (
        (isStack && newLayers === null) ||
        (isComposite && newProps === null)
      ) {
        const method = isStack ? '__getLayersFromStyle' : '__getPropsFromStyle';
        const parentItem = parentStyles.filter(
          (p) => prop[method](p.style) !== null
        )[0];
      ...
      canUpdate &&
        prop.__getFullValue() !== newValue &&
        prop.upValue(newValue, opt);
      ...
      if (isComposite) {
        ...
        if (prop.isDetached()) {
          const newStyle =
            prop.__getPropsFromStyle(style, { byName: true }) || {};
