...
export default class Property extends View {
  ...
  templateLabel(model) {
    ...
    return `
      ...
      ${
        !parent
          ? `<div class="${pfx}clear" style="display: none" ${clearProp}>${iconClose}</div>`
          : ''
      }
    `;
  }
  ...
  setValue(value) {
    const { model } = this;
    const result =
      isUndefined(value) || value === '' ? model.getDefaultValue() : value;
  ...
  render() {
    ...
    const clsType =
      type === 'number' ? `${pfx}${type} ${pfx}integer` : `${pfx}${type}`;
    ...
    el.className =
      `${className} ${clsType} ${className}__${name} ${cls}`.trim();
